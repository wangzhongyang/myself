...
func (s *SearchService) Do(ctx context.Context) (*SearchResult, error) {
	...
	ssss, _ := json.Marshal(body)
	fmt.Println("query body:		", string(ssss))
	fmt.Println("path:		", s.client.urls, path)
	...
	sssss, _ := res.Body.MarshalJSON()
	fmt.Println("res one:		", string(sssss))
